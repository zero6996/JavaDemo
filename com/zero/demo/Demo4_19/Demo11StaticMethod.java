...
如果没有static关键字，那么必须先创建对象，才能通过对象来使用它。
...
静态方法：类名称.静态方法()
 ...
 注意事项：
 1. 静态不能直接访问非静态。原因：因为在内存当中是[先]有的静态内容,[后]有的非静态内容。"前人不知后事，后人尽知前史"
...
public class Demo11StaticMethod {
    public static void main(String[] args) {
        ...
        //对于静态方法来说，可以通过对象名称进行调用，也可以直接通过类名称来调用。
        obj.methodStatic(); // 不推荐，这种写法会被编译器会自动优化为"类名称.静态方法名"
        MyClass.methodStatic(); // 推荐写法

        //对于本类自己的方法，可以省略类名称直接调用。
        myMethod();
        Demo11StaticMethod.myMethod(); // 完全等效于上面的调用方法
    }
